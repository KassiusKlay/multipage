...
def get_drop_df(df, sp500):
    ...
    for i in range(len(years) - 1):
        ...
        recover_idx = df[min_idx:end_idx][
                df[min_idx:end_idx] > df[start_idx]].index[0]
    ...
    drop_df['pct_drop'] = drop_df[
            ['price_start', 'price_min']].pct_change(
                    axis='columns')['price_min']
    drop_df['pct_gain'] = drop_df[
            ['price_min', 'price_end']].pct_change(axis='columns')['price_end']
    ...
    drop_df['days_to_min'] = (
            drop_df.min_idx - drop_df.start_idx).apply(lambda x: x.days)
    drop_df['days_to_recover'] = (
            drop_df.recover_idx - drop_df.min_idx).apply(lambda x: x.days).\
        fillna(0).astype(int)
    drop_df['days_to_new_peak_from_bottom'] = (
            drop_df.end_idx - drop_df.min_idx).apply(lambda x: x.days)
    drop_df['days_to_new_peak_from_previous'] = (
            drop_df.end_idx - drop_df.start_idx).apply(lambda x: x.days)
...
sp500 = get_ticker_data('SPY').history(
        start=start_date).loc[df.index.min():, 'Close']

drop_df = get_drop_df(df, sp500)

layers = alt.layer()
layers += alt.Chart(df.reset_index()).mark_line().encode(
        x="Date:T",
        y="Close:Q",
        )
for _, row in drop_df.iterrows():
    layers += alt.Chart(
            df[row.start_idx: row.min_idx].reset_index()
            ).mark_line(color='orange').encode(
        x="Date:T",
        y="Close:Q",
        tooltip=alt.value(f'{row.pct_drop:.0%}')
        )

st.title(f'{ticker}')
with st.expander('Explicação (clique para abrir)'):
    st.markdown('''
     - No grafico
        - A **LARANJA** as quedas superiores a 10%
        - Apontar com o rato para o tracejado laranja para saber o valor
        - O grafico e interactivo (mexer e ampliar com o rato)
            - Duplo-clique para voltar ao inicio
     - Primeira tabela
        - Cada linha corresponde a uma quebra assinalada no grafico
     - Segunda tabela
        - Minimo, Maximo e Media dos valores
        ''')
st.altair_chart(layers.interactive(), use_container_width=True)

st.dataframe(drop_df)

st.write(drop_df[:-1].describe().loc[
    ['min', 'max', 'mean'],
    ['pct_drop', 'pct_gain', 'sp500_change',
        'days_to_min', 'days_to_recover',
...
combined_pct = pd.concat([
    sp500_pct, ticker_pct], axis=0)

nearest = alt.selection(type='single', nearest=True, on='mouseover',
                        fields=['Date'], empty='none')
selectors = alt.Chart(combined_pct.reset_index()).mark_point().encode(
        x='Date:T',
        opacity=alt.value(0),
        ).add_selection(nearest)

line = alt.Chart(combined_pct.reset_index()).mark_line().encode(
        x='Date:T',
...
rules = alt.Chart(combined_pct.reset_index()).mark_rule(color='gray').encode(
    x='Date:T',
...
layer = alt.layer(line, selectors, points, rules, text).interactive()
